...
let pixelDistance = 0
let distanceShift = 0
let distanceMeasurement = "undefined measurement"
let distanceShiftChangeLogContainer = null
...
let maxValueFromAllPixelsPrev = null
...
socket.on('data-income', (data) => {
    ...
    if(maxValueFromAllPixelsPrev !== maxValueFromAllPixels) {
        if(maxValueFromAllPixelsPrev !== null) {

            // calculate shifting
            distanceShift = maxValueFromAllPixels - maxValueFromAllPixelsPrev
            // if shift is negative , multiply it with -1, to get positive number
            distanceShift<0?distanceShift*=-1:""

            // multiply it also with pixel distance, specified from user
            distanceShift = distanceShift * pixelDistance;

            console.log(`MAX VALUE CHANGE DETECTED FROM-> ${maxValueFromAllPixelsPrev} to -> ${maxValueFromAllPixels}`)
            console.log(`SHIFT IS -> ${distanceShift}`)

            let loggingData = '<span class="shadow-2 log-line">CHANGE DETECTED __FROM__->__TO__, __SHIFT_SIZE__ __MEASURE__: AT __DATE__</span>'

            let dateText = moment().format();

            loggingData = loggingData.replace('__FROM__', maxValueFromAllPixelsPrev)
            loggingData = loggingData.replace('__TO__', maxValueFromAllPixels)
            loggingData = loggingData.replace('__SHIFT_SIZE__', distanceShift)
            loggingData = loggingData.replace('__MEASURE__', distanceMeasurement)
            loggingData = loggingData.replace('__DATE__', dateText)

            distanceShiftChangeLogContainer.innerHTML += loggingData
            distanceShiftChangeLogContainer.scrollTop = distanceShiftChangeLogContainer.scrollHeight;
        }
        maxValueFromAllPixelsPrev = maxValueFromAllPixels
    }
...
function selectPort(){
    ...
    pixelDistance = document.getElementById('pixel_distance').value
    distanceMeasurement = document.getElementById('distance_measurement').value
    distanceShiftChangeLogContainer = document.getElementById('distance_shift_container')
